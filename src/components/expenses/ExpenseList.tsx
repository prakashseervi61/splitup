...
export default function ExpenseList({ expenses, loading, error, groupId, members, onRefresh, userId }: ExpenseListProps) {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="py-8 text-center text-sm text-text-muted">Loading expenses...</div>
      ) : expenses.length === 0 ? (
        ...
        <div className="space-y-2">
          {expenses.map((exp) => (
            <div
              key={exp.id}
              className="flex items-center gap-4 rounded-xl border border-border bg-surface px-4 py-3 shadow-sm transition-colors hover:border-divider"
            >
